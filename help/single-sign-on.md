...
>[!CAUTION]
>
>Este artículo describe el inicio de sesión único (SSO) y CRM User Management avanzado. Si su cuenta se aprovisionó **después del 10/9/2020**, haga caso omiso de este artículo, ya que Identity Management y el SSO se configurarán dentro de [Adobe Admin Console para su [!DNL Marketo Measure] integración](/help/implementation-guide.md).
...
Configure la aplicación de [!DNL Marketo Measure] en su proveedor de identidad. Consulte la documentación externa que se muestra a continuación para ver tutoriales.
    ...
    a. Cuando se le pida la URL de inicio de sesión único, la URL del destinatario o la URL de destino, la URL del servicio de atención al cliente de aserción de SAML (ACS), use [https://apps.bizible.com/BizibleSAML2/ReceiveSSORequest](https://apps.bizible.com/BizibleSAML2/ReceiveSSORequest)
    
    b. Cuando se le pida la URL de restricción de audiencia o el identificador único definido por la aplicación, use [https://BizibleLPM](https://biziblelpm/)

Cambie a SSO personalizado en la aplicación de [!DNL Marketo Measure]

    a. Una vez que el grupo de facturación se haya habilitado para tu cuenta, ahora puedes navegar a [!UICONTROL Configuración] >>[!UICONTROL Seguridad] >> [!UICONTROL Autenticación]
    
    b. De manera predeterminada, el tipo de inicio de sesión se establecerá en &quot;Usuarios de CRM&quot;.
...
Rellene la configuración de conexión de la configuración del proveedor de identidad
    ...
    a. El proveedor de identidad puede proporcionar un documento .xml de metadatos de IdP que extraerá los campos de configuración requeridos. Cargue el contenido del documento .xml o complete los tres campos a continuación con el resultado obtenido durante el proceso de configuración del proveedor de identidad. **No es necesario completar ambos.**
    
    i. Dirección URL de IdP: Dirección URL a la que [!DNL Marketo Measure] debe apuntar para autenticar a los usuarios en la aplicación [!DNL Marketo Measure] id. A veces se denomina &quot;URL de redireccionamiento&quot;.
...
Configure y asigne la configuración de atributos del usuario a los campos Nombre, Apellidos y Dirección de correo electrónico correspondientes.
    ...
    a. Al escribir los atributos de SAML, [!DNL Marketo Measure] podrá reconocer a los usuarios por la información pasada.
...
Configure y asigne la configuración de la función de usuario a las funciones o grupos respectivos clasificados de su IdP.
    ...
    a. Los clientes tienen la opción de asignar  [!DNL Marketo Measure] funciones de usuario según los grupos definidos en su proveedor de identidad. Al introducir sus atributos de SAML, [!DNL Marketo Measure]  podrá asignar los roles y grupos de su usuario a los permisos de usuario de  [!DNL Marketo Measure] . Le recomendamos encarecidamente que configure estas funciones para que su  [!DNL Marketo Measure] administrador tenga derechos suficientes para actualizar su cuenta.
    ...
    i. [!DNL Marketo Measure] Usuario estándar: proporcione el valor de rol o grupo (de su proveedor de SSO) para los usuarios que deben tener acceso de solo lectura a la [!DNL Marketo Measure] aplicación.
    ii. [!DNL Marketo Measure] Usuario administrador de cuentas: proporcione el valor de rol o grupo (de su proveedor de SSO) para los usuarios que deben tener acceso administrativo a la [!DNL Marketo Measure] aplicación. Esto significa que el rol tiene acceso para cambiar configuraciones y valores relacionados con su cuenta.
    iii. Debe tener un atributo en su IdP con el nombre exacto de &quot;grupos&quot; que contenga los valores que puso en los atributos &quot;Usuario estándar de Bizible&quot; o &quot;Usuario administrador de cuenta de Bizible&quot;.
    
    c. Si se deben asignar varios roles o grupos a un rol, escriba cada valor separado por una coma.

![c. Si se deben asignar varios roles o grupos a un rol,](assets/discover-control-2.png)

Prueba de la configuración de inicio de sesión único

    a. Antes de pulsar Guardar, se le pedirá que haga clic en el botón [!UICONTROL Probar autenticación SAML] para comprobar que la configuración es correcta.
...
Pruébelo.
    ...
    a. Use su nueva dirección URL de inicio de sesión personalizada e intente volver a iniciar sesión en la  [!DNL Marketo Measure] aplicación con las credenciales de su proveedor de identidad.
...
![Para los roles de Salesforce, use el nombre de cada rol. Todos los roles](assets/discover-control-3.png)
...
![Para los roles de Dynamics, use el nombre de cada rol de seguridad. Todo](assets/discover-control-3.png)
...
![Una vez configurado el SSO personalizado, la página de usuarios es](assets/discover-control-3.png)

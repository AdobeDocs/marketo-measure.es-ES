...
description: 'Configuración de canal personalizado en línea: [!DNL Marketo Measure]'
...
Para tener informes precisos, los canales de marketing deben configurarse para reflejar la estrategia de UTM de su organización. Esta guía le explica la mejor manera de configurar las reglas de canal personalizadas.
...
* **Campaña:** añada nombres de campaña aquí, independientemente de si el valor proviene de UTM o de Campañas de Salesforce para [!DNL Marketo Measure] Funcionalidad de actividades
...
La siguiente regla solo pide el parámetro medium, por lo que los datos con ese parámetro se agrupan en este canal. Por último, para [!DNL Facebook]Sin embargo, cualquier dato proveniente de la URL de Facebook se coloca en el último bloque de Facebook.
...
Debido a [!DNL Marketo Measure] lógica que funciona de arriba a abajo, la regla comodín, indicada con un asterisco (&#42;), debe colocarse al final de la hoja de reglas. Todos los datos que no se capturan ni se ordenan según las demás reglas se añaden a este bloque de comodines.
...
Además, tenga en cuenta que si crea un subcanal para uno de los canales, debe crear un subcanal para todas las reglas debajo de ese canal. En otras palabras, si crea un subcanal, no puede dejar en blanco el resto de las columnas.
...
* No agregue ningún carácter especial a los nombres de canal (por ejemplo, $%#&amp;&#42;@)
...
* No se pueden crear reglas basadas en subdominios diferentes.
...
* No es necesario que agregue punto-com (.com) al final de la dirección URL de referencia.
...
Asegúrese de que los nuevos valores de canal y subcanal que está agregando en el CSV ya se hayan agregado al área de configuración de canal de su cuenta de Bizible. Compruebe si todos los nombres de canal y subcanal coinciden en el CSV con el área de configuración de canal de su cuenta de [!DNL Marketo Measure]. Asegúrese de comprobar la existencia de comas y espacios.
